...
void TextureManager::load(DXcore* core, std::string filename)
{
...
void Sampler::bind(DXcore* core, UINT startSlot) {
    core->devicecontext->PSSetSamplers(startSlot, 1, &state);
}

ID3D11ShaderResourceView* TextureManager::find(std::string name)
{
